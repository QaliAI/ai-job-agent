...
def load_candidate_profile(candidate_dir: str = "candidate") -> Dict[str, Any]:
    ...
    if os.path.exists(master_path):
        try:
            ...
            t_match = re.search(r"Current / Target Title\s*:\s*([^\n\r]+)", m_text, re.IGNORECASE)
            if not t_match:
                t_match = re.search(r"Current Title\s*:\s*([^\n\r]+)", m_text, re.IGNORECASE)
            if t_match:
                profile_data["title"] = t_match.group(1).strip()

            exp_match = re.search(r"Total Years of Experience\s*:\s*(\d+)", m_text, re.IGNORECASE)
            if exp_match:
                profile_data["years_exp"] = int(exp_match.group(1))
...
def score_single_job(
    ...
    title_score = 40
    target_titles = profile.get("target_titles", []) or [profile.get("title", "")]
    for target in target_titles:
        t_words = [w.lower() for w in target.split() if len(w) > 2]
        matches = sum(1 for w in t_words if w in title_lower)
        if matches == len(t_words):
            title_score = 100
            break
        elif matches > 0:
    ...
    cand_exp = profile.get("years_exp", 5)
    ...
    cand_mode = profile.get("work_mode_pref", "remote")
    if cand_mode == "remote":
